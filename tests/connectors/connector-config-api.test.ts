...
describe("connector config api", () => {
  ...
  it("requires admin auth when token is configured and never returns plaintext appSecret", async () => {
    const db = createSqliteDatabase(":memory:");
    migrateDatabase(db);
    ...
    const openRead = await app.inject({
      method: "GET",
      url: "/api/connectors/feishu/config"
    });
    expect(openRead.statusCode).toBe(200);
    expect(openRead.json().appSecret).toBeUndefined();
    ...
    expect(deniedUpdate.statusCode).toBe(401);
